...
const other_protocols_1 = __importDefault(require("../market/other-protocols"));
...
(function (CronJob) {
    ...
    CronJob.crawlOtherProtocolsAPY = new cron_1.default.CronJob({
        cronTime: constants_1.cronTime.perHour10,
        onTick: async () => {
            let result = await other_protocols_1.default.queryLendingAPY();
            let cosmwasmClient = await cosmwasm_1.default.getCosmWasmClient();
            ...
            // console.log(data);
            await market_data_repository_1.default.createOrUpdate(constants_1.MARKET_DATA_TYPE.OTHER_PROTOCOLS_APY, data, []);
        },
        onComplete: () => { },
        utcOffset: +7,
        runOnInit: false,
    });
    CronJob.crawlOtherProtocolsLiquidationList = new cron_1.default.CronJob({
        cronTime: constants_1.cronTime.perHour30,
        onTick: async () => {
            let liquidationList = await other_protocols_1.default.queryLiquidationList();
            await market_data_repository_1.default.createOrUpdate(constants_1.MARKET_DATA_TYPE.OTHER_PROTOCOLS_LIQUIDATION_LIST + "_aave", liquidationList.aave, []);
            await market_data_repository_1.default.createOrUpdate(constants_1.MARKET_DATA_TYPE.OTHER_PROTOCOLS_LIQUIDATION_LIST + "_venus", liquidationList.venus, []);
        },
        onComplete: () => { },
        utcOffset: +7,
        runOnInit: true,
    });
    CronJob.crawlMoneyMarketInfoMessage = new cron_1.default.CronJob({
        cronTime: constants_1.cronTime.perHour10,
        onTick: async () => {
            let cosmwasmClient = await cosmwasm_1.default.getCosmWasmClient();
            let marketInfo = await orchai_lending_1.default.queryMarketInfo(cosmwasmClient);
    ...
    async function start() {
        CronJob.crawlOtherProtocolsAPY.start();
        CronJob.crawlMoneyMarketInfoMessage.start();

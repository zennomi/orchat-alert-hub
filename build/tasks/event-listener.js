    ...
    function executeOrderbookPairListener(cosmwasmClient) {
        ...
        webSocketClient.onmessage = async (message) => {
            ...
            if (!isEmpty(result)) {
                ...
                let events = transaction.result.events;
                let attributes = filterEvents(events, "wasm");
                let filteredAttributes = filterAttributesKey(attributes, "list_order_matched");
                let listOrderMatchedStr = base64ToText(filteredAttributes[0].value);
                listOrderMatchedStr = listOrderMatchedStr.replaceAll("Attribute", "");
                listOrderMatchedStr = listOrderMatchedStr.replaceAll("key", '"key"');
                listOrderMatchedStr = listOrderMatchedStr.replaceAll("value", '"value"');
